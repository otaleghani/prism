...
An interactive command-line utility for uninstalling packages from the current user's environment. It is designed specifically for packages installed "imperatively" via `nix profile` or [[14 prism-install]], rather than those defined declaratively in the system configuration. It provides a searchable, multi-select interface to easily identify and remove software.
