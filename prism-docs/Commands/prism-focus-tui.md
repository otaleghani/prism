...
A smart wrapper script designed to enforce a "single-instance" policy for Terminal User Interface (TUI) applications. When you attempt to launch a TUI tool (like [[btop]], [[neovim]], or [[yazi]]) using this script, it checks if an instance is already running.
...
This script is typically used in keybindings or application launchers ([[Rofi]]) to ensure consistent behavior for frequently used tools.
