...
A configuration utility for managing monitor settings in [[Hyprland]] (resolution, positioning, scaling). It simplifies the process by opening the monitor configuration file in your default text editor and automatically applying the changes once you close the editor. It also offers an optional step to persist these changes to your Prism OS configuration overrides.
...
2. **Editing:**
    - It opens `$HOME/.config/hypr/monitors.conf` using your system's default editor ([[neovim]]).
